...
bookmarkContainer.addEventListener('click', () => {
  if (bookmark.className == 'img-container unbookmarked' && para.className == 'unactivated') {
...
const openWindow = obj => {
  ...
  obj.parentElement.parentElement.style.border = '1px solid hsl(176, 50%, 47%)';
};
const closeWindow = obj => {
  obj.previousElementSibling.style.opacity = 0;
  obj.parentElement.parentElement.style.height = '200px';
  obj.parentElement.parentElement.style.border = '1px solid hsl(0, 0%, 75%)';
};
const closeAll = () => {
